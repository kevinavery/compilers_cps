...
;; Essentially Matt's cps-header with extra testing
...
(define transform cps-transform-program)

;; Runs transformer on test name and prints diff result
(define (run-test try_interpret? name)
  (let* ([from-file-name   (string-append name ".desugar")]
         [to-file-name     (string-append name ".cps")]
